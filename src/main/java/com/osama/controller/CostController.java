...
import com.osama.model.Cost;
import com.osama.view.ExcelReport;
import com.osama.view.PdfReport;
...
public class CostController {
    ...
    public ModelAndView getCalc(HttpServletRequest request, @ModelAttribute("cost") @Valid Cost cost, BindingResult result){
        if (result.hasErrors()) {

            return new ModelAndView("inputForm","cost",cost);
        }
...
//        allCost.setSalaryHours(cost.getSalaryHours());
//        allCost.setWaterHours(cost.getWaterHours());
       ...
       allCost.setVehicleRoadTax(vehicleRoadTax);
        ...
        if(allCost.getSalariesString().equals("SALARIES - DRIVERS (20% of revenue)")){
        ...
        return new ModelAndView("result","cost",cost);

    }

    @ModelAttribute("cost")
    public Cost formBackingObject() {
        return new Cost();
    }
}
